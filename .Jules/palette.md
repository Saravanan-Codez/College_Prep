...
## 2026-09-25 - Screen Reader Support for Buttons with Mixed Emojis and Shorthand
**Learning:** Found an accessibility issue pattern where buttons containing a mixture of emojis and fragmented shorthand text (like flashcard intervals: '😖 Again 1d') lacked explicit `aria-label`s. Screen readers announce the literal emoji description alongside the shorthand, resulting in disjointed and confusing audio (e.g., "Confounded face Again 1d button").
**Action:** When implementing buttons that rely on emojis mixed with fragmented shorthand text for visual layout, always provide a clear, descriptive `aria-label` attribute (e.g., `aria-label="Rate Again, review in 1 day"`) to ensure screen readers announce the intended action rather than a literal, confusing description.
