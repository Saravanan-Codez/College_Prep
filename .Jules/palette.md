...
## 2024-11-20 - ARIA Logging Roles for Dynamic Text Containers
**Learning:** Found an accessibility issue pattern where dynamic text containers like chat windows and terminal outputs lacked proper ARIA roles to notify screen readers of new content, forcing users to manually read through the entire log to find updates.
**Action:** When implementing dynamic text containers (such as chat windows or terminal outputs), always include `role="log"` and `aria-live="polite"` (optionally with `aria-atomic="false"` and `aria-relevant="additions text"`) to ensure newly appended content is announced automatically by screen readers without user interaction.
